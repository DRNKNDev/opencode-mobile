...
export function useModels(): UseModelsReturn {
  const { models: connectionModels, isConnected, refreshModels: refreshConnectionModels } = useConnectionContext()
  ...
  const getProviderIdForModel = useCallback((modelId: string): string | null => {
  ...
  const getEffectiveDefaultModel = useCallback((availableModels: Model[]) => {
    if (availableModels.length === 0) return null

    // Priority 1: Last used model
    if (modelPreferences.lastUsedModel) {
      const lastUsed = availableModels.find(m => m.id === modelPreferences.lastUsedModel)
      if (lastUsed) return lastUsed.id
    }

    // Priority 2: Default model (from API or user setting)
    if (modelPreferences.defaultModel) {
      const defaultModel = availableModels.find(m => m.id === modelPreferences.defaultModel)
  ...
  useEffect(() => {
    if (isConnected && connectionModels.length > 0) {
      ...
      if (isInitialized && !storage.getSelectedModel() && connectionModels.length > 0) {
  ...
  const getModelById = useCallback((modelId: string): Model | null => {
