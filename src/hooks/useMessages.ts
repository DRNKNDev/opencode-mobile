...
export function useMessages(sessionId: string | null): UseMessagesReturn {
  ...
  const loadMessages = useCallback(
    async (sessionId: string, forceSync = false) => {
      console.log(
        `🔄 Loading messages for session: ${sessionId}${forceSync ? ' (forced sync)' : ''}`
      )
      ...
      try {
        if (connectionService.isReady()) {
          ...
          console.log(
            `✅ Received ${remoteMessages.length} messages from server`
          )

          // Only update if server has different/newer messages
          const messagesChanged =
            remoteMessages.length !== cachedMessages.length ||
            remoteMessages.some((remoteMsg, index) => {
              ...
              return (
                ...
                Math.abs(
                  remoteMsg.timestamp.getTime() - cachedMsg.timestamp.getTime()
                ) > 1000
              ) // Allow 1s difference
            })
